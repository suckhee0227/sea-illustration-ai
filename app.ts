...
interface IAppData {
  ...
  trainCount: number;
  oceanTotal: number;
  fishPalettes: Array<{ body: string; accent: string; fin: string }>;
}
...
interface ITextData {
  ...
  oceanIntroText: string;
  runBtn: string;
  resultTitle: string;
  restartBtn: string;
  dykFacts: string[];
  trainMessages: Record<string, string>;
}

interface IAssetList {
  images: Array<{ id: string; file_path: string; width: number; height: number; isBackgroundImage: boolean; isNewAsset: boolean; description: string }>;
  sounds: Array<{ id: string; file_path: string; duration_seconds: number; volume: number; isBackgroundMusic: boolean; isNewAsset: boolean; description: string }>;
}
...
interface FishObject {
  ...
  hasStripes: boolean;
  hasSpots: boolean;
  spotPositions: Array<{ x: number; y: number }> | null;
  features: number[];
  // ocean fields
  oceanX?: number;
  oceanY?: number;
  direction?: number;
  oceanSpeedBase?: number;
  oceanBob?: number;
  oceanBobSpeed?: number;
  judged?: boolean;
  verdict?: { label: string; confidence: number } | null;
  verdictTimer?: number;
  lingerTimer?: number;
  exiting?: boolean;
}
...
type GameObject = FishObject | TrashObject;
...
let bgCached: HTMLCanvasElement | null = null;
let bg2Image: HTMLImageElement | null = null;
let bg2Cached: HTMLCanvasElement | null = null;
let robotImg: HTMLImageElement | null = null;
let robotOpenImg: HTMLImageElement | null = null;
let rCheckImg: HTMLImageElement | null = null;
let rFailImg: HTMLImageElement | null = null;
let robotBodyCanvas: HTMLCanvasElement | null = null;
let robotDomeCanvas: HTMLCanvasElement | null = null;
let robotOpenProgress = 0;   // 0=닫힘, 1=열림
let robotOpenTarget = 0;     // 목표값
let robotOpenCloseTimeout: number | null = null;
let fishImages: HTMLImageElement[] = [];
let trashImages: HTMLImageElement[] = [];
let trashLabels: string[] = [];
...
const TRASH_FILES: Array<{ file: string; label: string }> = [
...
type Stage = "intro" | "train" | "ocean-intro" | "ocean-run" | "review" | "result";
let stage: Stage = "intro";
let introStepIndex = 0;
let currentIndex = 0;
let trainingData: TrainingDatum[] = [];
let oceanResults: OceanResult[] = [];
...
let oceanObjects: GameObject[] = [];
let oceanBubbles: Bubble[] = [];
let oceanTime = 0;
let bgHills: Array<{ cx: number; cy: number; rx: number; ry: number }> = [];
let fgHills: Array<{ cx: number; cy: number; rx: number; ry: number }> = [];
let robotX = 0;
let robotY = 0;
let robotTargetX = 0;
let scanTargetObject: GameObject | null = null;
let scanProgress = 0;
let robotEyeColor = "#4dd0e1";
let robotEyeTimer = 0;

// 바다 단계: 하나씩 순서대로
let oceanQueue: GameObject[] = [];
let oceanQueueIndex = 0;
let oceanObjX = 0;
let oceanObjTargetX = 0;
let oceanPhase: "entering" | "scanning" | "judged" | "leaving" | "idle" | "done" = "idle";
let oceanScanProg = 0;
let oceanJudgeTimer = 0;
let oceanCurrentVerdict: { label: string; confidence: number } | null = null;

let dykShownIndices = new Set<number>();
let introFrameObjects: GameObject[] = [];
...
let classifySound: HTMLAudioElement | null = null;
let enterSound: HTMLAudioElement | null = null;
let laserSound: HTMLAudioElement | null = null;
let poyoSound: HTMLAudioElement | null = null;
let introAdvanceLocked = false;

// DOM refs
let titleEl: HTMLElement;
let counterNum: HTMLElement;
let introBubble: HTMLElement;
let continueBtn: HTMLButtonElement;
let classifyButtons: HTMLElement;
let btnYes: HTMLButtonElement;
let btnNo: HTMLButtonElement;
let didYouKnow: HTMLElement;
let dykFact: HTMLElement;
let dykBtn: HTMLButtonElement;
let oceanIntroEl: HTMLElement;
let bigArrow: HTMLElement;
...
let oceanContinueBtn: HTMLButtonElement;
let resultOverlay: HTMLElement;
let resultScore: HTMLElement;
let resultDetail: HTMLElement;
let restartBtn: HTMLButtonElement;
let meetAiTooltip: HTMLElement;
// Review (2-3 page)
let reviewBottom: HTMLElement;
let reviewBubble: HTMLElement;
let reviewTrainBtn: HTMLElement;
let reviewContinueBtn: HTMLElement;
let reviewToggle: HTMLElement;
let reviewFishBtn: HTMLElement;
let reviewTrashBtn: HTMLElement;
let reviewShowFish = true;
let reviewStep = 0;
let reviewObjects: Array<{ obj: GameObject; predicted: string }> = [];
let reviewFishObjects: Array<{ obj: GameObject; predicted: string }> = [];
let reviewTrashObjects: Array<{ obj: GameObject; predicted: string }> = [];
...
function drawPhotoFrame(cx: number, cy: number, size: number, showScanner: boolean) {
...
function drawRobot(
  x: number, y: number, size: number, eyeColor: string,
  showScanBeam: boolean, scanTargetX: number | null, scanTargetY: number | null
) {
  // 스캔 빔
  ...
  if (robotImg && robotBodyCanvas && robotDomeCanvas) {
    ...
    ctx.drawImage(robotBodyCanvas, 0, 0, robotBodyCanvas.width, robotBodyCanvas.height,
      baseX, baseY, fullW, fullH);
    ...
    ctx.rotate(angle);
    ctx.translate(-pivotX, -pivotY);
    ctx.drawImage(robotDomeCanvas, 0, 0, robotDomeCanvas.width, robotDomeCanvas.height,
      domeDrawX, baseY, domeDrawW, domeDrawH);
    ctx.restore();
...
function drawFish(obj: FishObject, cx: number, cy: number, baseSize: number) {
...
function drawTrash(obj: TrashObject, cx: number, cy: number, baseSize: number, extraRotation: number) {
...
function createTrash(): TrashObject {
...
function computeFeatures(obj: GameObject): number[] {
  if (obj.type === "fish") {
...
function generateObject(): GameObject {
...
function predict(features: number[]): { label: string; confidence: number } | null {
  // 학습 데이터 없으면 판단 불가
  ...
  let fishCount = 0;
  near.forEach((n) => { if (n.label === "fish") fishCount++; });
  ...
  if (active && Math.abs(active.x - active.targetX) < 20 && !hasLeaving && !trainReady && !messageShowing) {
    ...
    if (tMsg && !shownMessageRounds.has(roundNum)) {
      ...
      if (dykIdx >= 0) {
        ...
        // 팝업 이미지 설정
        const dykImage = document.getElementById("dyk-image") as HTMLImageElement;
        const dykImages = [
...
function getActiveTrainObject(): GameObject | null {
  const active = trainQueue.find((q) => q.state === "active");
...
let oceanItemVerdicts: Array<{ label: string; confidence: number } | null> = [];
let oceanItemStates: Array<"waiting" | "scanning" | "judged"> = [];
let oceanItemFallY: number[] = [];
let oceanItemFallSpeed: number[] = [];
let oceanItemFallX: number[] = [];
...
function updateOceanSingle() {
  if (paused) return;
  if (oceanPhase === "done" || oceanPhase === "idle") return;
  ...
  if (robotEyeTimer > 0) {
    robotEyeTimer--;
    if (robotEyeTimer === 0) robotEyeColor = "#4dd0e1";
  }
...
function drawOceanSingle() {
  ...
  if (oceanPhase !== "done") {
    ...
    let bc = "77, 208, 225";
    if (oceanBeamColor === "green") bc = "102, 187, 106";
    else if (oceanBeamColor === "red") bc = "239, 83, 80";
  ...
  for (let i = 0; i < oceanQueue.length; i++) {
    ...
    if (itemX < -objSize * 2 || itemX > W + objSize * 2) continue;
    ...
    // 화면 아래로 떨어져나간 객체 스킵
    if (drawY > H + objSize) continue;

    // 객체 그리기
    if (obj.type === "fish") {
      drawFish(obj as FishObject, drawItemX, drawY, objSize);
    } else {
      drawTrash(obj as TrashObject, drawItemX, drawY, objSize, 0);
    }
    ...
    if (hasVerdict) {
      let boxColor: string;
      let badgeType: "check" | "ban" | "question";
...
function startTypewriter(text: string, element: HTMLElement) {
  // 이전 타자기 중지
...
function setupIntroStep(step: number) {
  introStepIndex = step;
...
function handleAnswer(userLabel: string) {
  if (stage !== "train" || !trainReady) return;
  const activeObj = getActiveTrainObject();
  if (!activeObj) return;

  if (classifySound) { classifySound.currentTime = 0; classifySound.play().catch(() => {}); }
  btnYes.disabled = true;
  btnNo.disabled = true;
  trainReady = false;
...
let pendingTrainMsg: string | null = null;
let pendingDykIndex = -1;
let shownMessageRounds = new Set<number>();
let messageShowing = false;

function showTrainMessage(msg: string) {
  // 버튼 숨기기 + 오버레이 활성화
  classifyButtons.classList.add("hidden");
...
function showReview() {
  ...
  (document.querySelector(".top-ui") as HTMLElement).classList.add("hidden");
  ...
  // 화면 클릭으로 메시지 진행
  const reviewClickHandler = (e: MouseEvent) => {
    if ((e.target as HTMLElement).closest(".review-toggle") || (e.target as HTMLElement).closest(".review-train-btn"))
      return;
    if ((e.target as HTMLElement).closest(".review-continue-btn"))
      return;
    advanceReviewStep();
  };
  document.getElementById("appContainer")!.addEventListener("click", reviewClickHandler);
  (window as any)._reviewClickHandler = reviewClickHandler;
}

function advanceReviewStep() {
  reviewStep++;
  if (reviewStep >= REVIEW_MESSAGES.length) {
    if ((window as any)._reviewClickHandler) {
      document.getElementById("appContainer")!.removeEventListener("click", (window as any)._reviewClickHandler);
      (window as any)._reviewClickHandler = null;
    }
  ...
  for (let r = 0; r < 3; r++) {
    ...
    for (let c = 0; c < count && idx < items.length; c++, idx++) {
      ...
      if (item.obj.type === "fish") {
        drawFish(item.obj as FishObject, x + swayX, baseY + floatY, objSize);
      } else {
        drawTrash(item.obj as TrashObject, x + swayX, baseY + floatY, objSize, 0);
      }
    }
  }
  ...
  if (stage === "review") {
    ...
    if (stage === "ocean-intro") {
      ...
      if (introObj.type === "fish") {
        drawFish(introObj as FishObject, introObjX, introObjY, Math.min(W, H) * 0.25);
      } else {
        drawTrash(introObj as TrashObject, introObjX, introObjY, Math.min(W, H) * 0.25, 0);
      }
    ...
    if (stage === "train") {
      ...
      sorted.forEach((q) => {
        const frameX = cx + q.x;
        const frameY = cy + q.y;
        if (frameX < -frameSize || frameX > W + frameSize) return;
        if (q.scale < 0.05) return;
        ...
        ctx.clip();
        if (q.obj.type === "fish") {
          drawFish(q.obj as FishObject, frameX, frameY, frameSize * 0.8);
        } else {
          drawTrash(q.obj as TrashObject, frameX, frameY, frameSize * 0.8, 0);
        }
        ctx.restore();
        ctx.restore();
      });
      ...
      introPositions.forEach((pos, i) => {
        ...
        ctx.clip();
        if (obj.type === "fish") {
          drawFish(obj as FishObject, pos.x, cy, s * 0.8);
        } else {
          drawTrash(obj as TrashObject, pos.x, cy, s * 0.8, 0);
        }
        ctx.restore();
      });
    }
...
function bindEvents() {
  continueBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    if (stage === "intro") advanceIntro();
    else if (stage === "train") showOceanIntro();
  });
  ...
  reviewTrainBtn.addEventListener("click", (e) => {
    ...
    classifyButtons.classList.remove("hidden");
    continueBtn.classList.remove("hidden");
    (document.querySelector(".top-ui") as HTMLElement).classList.remove("hidden");
    updateCounter();
  });
  ...
  // 재생 컨트롤 아이콘 그리기
  function drawBtnIcon(btn: HTMLElement, drawFn: (c: CanvasRenderingContext2D, s: number) => void) {
    const cv = btn.querySelector("canvas") as HTMLCanvasElement;
    if (!cv) return;
  ...
  function drawRewindIcon(c: CanvasRenderingContext2D, s: number) {
    const m = s / 2;
    // 왼쪽 삼각형
    c.beginPath();
    c.moveTo(m, m - 7);
  ...
  function drawPauseIcon(c: CanvasRenderingContext2D, s: number) {
    const m = s / 2;
    c.fillRect(m - 5, m - 7, 4, 14);
    c.fillRect(m + 1, m - 7, 4, 14);
  }

  function drawPlayIcon(c: CanvasRenderingContext2D, s: number) {
    const m = s / 2;
    c.beginPath();
    c.moveTo(m - 5, m - 8);
    c.lineTo(m + 7, m);
  ...
  function drawForwardIcon(c: CanvasRenderingContext2D, s: number) {
    const m = s / 2;
    c.beginPath();
    c.moveTo(m - 8, m - 7);
    c.lineTo(m, m);
  ...
  const trashImg = trashBtn.querySelector(".trash-img") as HTMLImageElement;
  const eraseConfirm = document.getElementById("erase-confirm")!;
  const eraseYes = document.getElementById("erase-yes") as HTMLButtonElement;
  const eraseNo = document.getElementById("erase-no") as HTMLButtonElement;
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "Enter") {
      if (stage === "intro") advanceIntro();
      else if (stage === "ocean-intro") startOceanRun();
      else if (!didYouKnow.classList.contains("hidden")) closeDidYouKnow();
    }
  });
}
...
export async function initApp() {
  ...
  btnYes = document.getElementById("btn-yes") as HTMLButtonElement;
  btnNo = document.getElementById("btn-no") as HTMLButtonElement;
  didYouKnow = document.getElementById("did-you-know")!;
  dykFact = document.getElementById("dyk-fact")!;
  dykBtn = document.getElementById("dyk-btn") as HTMLButtonElement;
  ...
  rewindBtn = document.getElementById("rewind-btn") as HTMLButtonElement;
  forwardBtn = document.getElementById("forward-btn") as HTMLButtonElement;
  oceanContinueBtn = document.getElementById("ocean-continue-btn") as HTMLButtonElement;
  ...
  fishImages = fishResults.filter((img): img is HTMLImageElement => img !== null);
  ...
  if (robotImg) {
    ...
    const domeMargin = Math.floor(natW * 0.25);
  ...
  navDots.forEach((dot) => {
    dot.addEventListener("click", () => {
      const page = Number((dot as HTMLElement).getAttribute("data-page"));
      if (!page) return;
